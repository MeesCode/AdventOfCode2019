# since we have been doing so much with intcode machines, this is one of few 
# instances where i tried to make the code somewhat pretty
...
class Machine:
  ...
  def run(self):
    while True:
      ...
      cmd = '%0*d' % (5, self.data[self.index])
      opcode = int(cmd[-2:])
      params = cmd[:3][::-1]
      ...
      for i in range(len(params)):
        if params[i] == '0':
          self.grow_mem(self.data[self.index + i + 1])
          p.append(self.data[self.index + i + 1])
        elif params[i] == '1':
          p.append(self.index + i + 1)
        else:
          self.grow_mem(self.relative_base + self.data[self.index + i + 1])
          p.append(self.relative_base + self.data[self.index + i + 1])
