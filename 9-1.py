...
class Machine:
  ...
  def run(self, inp=None):
    ...
    while True:
      ...
      if len(str(cmd)) >= 3:
        # direct mode
        if int(str(cmd)[-3:][0]) == 1:
          a = self.index+1
        # positional mode
        elif int(str(cmd)[-3:][0]) == 0:
          a = self.data[self.index+1]
        # relative mode
        elif int(str(cmd)[-3:][0]) == 2:
          a = self.relative_base + self.data[self.index+1]
      else:
        a = self.data[self.index+1]
      ...
      if i in [1,2,5,6,7,8]:
        if len(str(cmd)) >= 4:
          # direct mode
          if int(str(cmd)[-4:][0]) == 1:
            b = self.index+2
          # positional mode
          elif int(str(cmd)[-4:][0]) == 0:
            b = self.data[self.index+2]
          # relative mode
          elif int(str(cmd)[-4:][0]) == 2:
            b = self.relative_base + self.data[self.index+2]
        else:
          b = self.data[self.index+2]

      # third argument (can never be direct)
      if i in [1,2,3,7,8]:
        if len(str(cmd)) >= 5:
          # positional mode
          if int(str(cmd)[-5:][0]) == 0:

            c = self.data[self.index+3]
          # relative mode
          elif int(str(cmd)[-5:][0]) == 2:

            c = self.relative_base + self.data[self.index+3]
        else:
      ...
      if i == 1:
        ...
        print("output:", self.data[a])
        self.last_output = self.data[a]
        ...
        print("error on machine", self.name, self.index, self.data)
        return -1
