...
x, y = 0, 0
sizex, sizey = 0, 0
maxSizeX, minSizeX = 0, 0
maxSizeY, minSizeY = 0, 0
...
for wire in [wire1, wire2]:
    for i in wire:
        ...
        if(direction == 'R'):
            x += dist
            maxSizeX = max(maxSizeX, x)
        if(direction == 'L'):
            x -= dist
            minSizeX = min(minSizeX, x)
        if(direction == 'U'):
            y += dist
            maxSizeY = max(maxSizeY, y)
        if(direction == 'D'):
            y -= dist
            minSizeY = min(minSizeY, y)
    
    # print("x", minSizeX, maxSizeX)
    # print("y", minSizeY, maxSizeY)

# init 2d list
grid = [['.' for i in range(1+ maxSizeX + abs(minSizeX))] for j in range(1+maxSizeY + abs(minSizeY))]
steps = [[0 for i in range(1+ maxSizeX + abs(minSizeX))] for j in range(1+maxSizeY + abs(minSizeY))]
wires = [wire1, wire2]
...
def manhathan(originx, originy, newx, newy):
    return abs(originx - newx) + abs(originy - newy)
...
y = abs(minSizeY)
# print(collisions)
# collisions = sorted([manhathan(x, y, i[0], i[1]) for i in collisions])
collisions = sorted([steps[i[1]][i[0]] for i in collisions])
# print(collisions)
print(collisions[0])