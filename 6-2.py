...
class Node:
    ...
    def __str__(self):
        if self.orbits:
            return self.name + "->" + self.orbits.name
        else:
            return self.name
...
# create an entry for every node
for orbit in data:
    center = orbit.split(')')[0]
    planet = orbit.split(')')[1]

    for i in [center, planet]:
        if not findNode(i):
            nodes.append(Node(i))
...
for orbit in data:
    ...
    center = findNode(center)
    planet = findNode(planet)

    planet.orbits = center

# printNodes(nodes)

# this time we are looking for the first occurence of the same node 
me = findNode('YOU')
santa = findNode('SAN')
...
curNode = me
while curNode:
    meOrbits.append(curNode.name)
    curNode = curNode.orbits

curNode = santa
while curNode:
    santaOrbits.append(curNode.name)
    curNode = curNode.orbits

# find the first common orbit
common = ''
def findCommon(meOrbits, santaOrbits):
    for m in range(len(meOrbits)):
        for s in range(len(santaOrbits)):
            if meOrbits[m] == santaOrbits[s]:
                return meOrbits[m], (m-1) + (s-1)

common, steps = findCommon(meOrbits, santaOrbits)
# print(meOrbits)
# print(santaOrbits)
print(steps)